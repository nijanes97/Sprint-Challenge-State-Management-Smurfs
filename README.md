...
it helps to keep the state relatively clean
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
actions are processes that you can define for a reducer. reducers are a function that takes the previouse state and the action and returns the next state. store is the current state of the data that a program uses
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
application state is the immutable initial state that gets cloned, component state is the clone that gets changed
...
redux, it just makes more sense to me and I feel that I spent more time reading the docs and learning it
